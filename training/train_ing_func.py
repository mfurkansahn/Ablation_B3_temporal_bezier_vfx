...
def make_models_dict(models, opts, scores):
    model_dict = {
        'net_g': models['generator'].state_dict(),
        'optimizer_g': opts['optimizer_G'].state_dict(),
        'net_d': models['discriminator'].state_dict(),
        'optimizer_d': opts['optimizer_D'].state_dict(),
        'step': int(scores['step']),
        'iter_list': scores['iter_list'],
        'best_auc': float(scores['best_auc']),
        'auc_list': scores['auc_list'],
        'g_loss_list': scores['g_loss_list'],
        'd_loss_list': scores['d_loss_list'],

        # ✅ yeni alanlar: yoksa [] yaz
        'temp_list': scores.get('temp_list', []),
        'bez_list': scores.get('bez_list', []),
        'lam_t_list': scores.get('lam_t_list', []),
        'lam_b_list': scores.get('lam_b_list', []),
    }
    return model_dict
