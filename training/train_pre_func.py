...
def load_scores(cfg):
    # ✅ Load once (baseline uyumlu)
    ckpt = torch.load(cfg.resume) if cfg.resume else None

    if ckpt is not None:
        step       = int(ckpt.get('step', 0))
        iter_list  = ckpt.get('iter_list', [])
        best_auc   = float(ckpt.get('best_auc', 0.0))
        auc_list   = ckpt.get('auc_list', [])
        g_loss_list = ckpt.get('g_loss_list', [])
        d_loss_list = ckpt.get('d_loss_list', [])

        # ✅ Yeni alanlar: yoksa [] dön (baseline checkpoint ile crash olmaz)
        temp_list  = ckpt.get('temp_list', [])
        bez_list   = ckpt.get('bez_list', [])
        lam_t_list = ckpt.get('lam_t_list', [])
        lam_b_list = ckpt.get('lam_b_list', [])
    else:
        step = 0
        iter_list = []
        best_auc = 0.0
    ...
    scores['temp_list']  = temp_list
    scores['bez_list']   = bez_list
    scores['lam_t_list'] = lam_t_list
    scores['lam_b_list'] = lam_b_list
